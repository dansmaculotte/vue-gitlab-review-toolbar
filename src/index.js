module.exports = {
  install: function(Vue, options) {
    ...
    if (!options.projectPath || options.projectPath.length === 0) {
      console.warn("Please enter a Gitlab project path");
      return;
    }
    ...
    if (options.personalAccessToken && options.personalAccessToken.length) {
      ...
      try {
        localStorage.setItem("--gitlab-token", options.personalAccessToken);
      } catch (err) {
        return;
      }
    }
